...
/**
  ...
  Nous allons utiliser L'évènement 'request'. Il est documenté ici :
    https://nodejs.org/api/http.html#http_event_request

  Cet évènement survient lorsqu'une requête HTTP est reçue pas le serveur.

  On pourra alors écrire : httpServer.on('request', gestionnaireEvenement); où
...
httpServer.on('request', function (requeteHTTP, reponseHTTP) {
  ...
  **/
  ...
  /**
    ...
    local un message : 'ping'. Et lorsqu'il répond au client HTTP (le navigateur
    Internet), il enverra dans sa réponse un document en HTML contenant le texte
    'pong'.
  **/
  ...
  // On créé une variable qui contient le corps de la réponse sous la forme
  // d'octets.
  let corpsDeLaReponseHTTP = Buffer.from('pong');
  /**
    Buffer est une fonction/objet documenté ici :
  ...
  **/
  reponseHTTP.writeHead(200, {
    ...
    'Content-Length': corpsDeLaReponseHTTP.length
  });
  ...
  **/
  reponseHTTP.write(corpsDeLaReponseHTTP, function(){
    /**
      ...
      client HTTP. On utilise la méthode .end() de l'objet http.ServerResponse
...
/**
  Notre serveur doit être associé à un numéro identifiant : un port logiciel et
  démarré. Pour effectuer cette opération, on utilise la méthode .listen() de
  notre objet de type http.Server. Ici le port logiciel choisi est 8888. Notre
  serveur sera donc joignable (une fois démarré) à l'adresse :
    http://[adresse IP ou nom de domaine]:8888
**/
httpServer.listen(8888);

// Testez ce serveur HTTP avec votre navigateur Internet.
